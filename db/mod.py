...
db = cluster['HYG'];adv = db['adv'];ausen = db['ausente'];tick = db['tickets']
#-----------------------------------------------------------------------------------------------------#
async def advdb(id, qnt, motivo):

    adv.update_one(
        {"_id": id.id},
        {"$set": {f"Adv{qnt}": motivo}},
        upsert = True
    )
...
async def ausendb(id, motivo, data):
    ...
    ausen.update_one(
        {"_id": id.id}, 
        {"$set": {f"Nome": id.name, f"Motivo": motivo, f"Data": data, "Ausente?": True}}, 
        upsert = True
    )
...
async def desausendb(id):
    ...
    ausen.update_one(
        {"_id": id.id},
        {"$set": {f"Nome": id.name, f"Motivo": 'None', f"Data": 'None', "Ausente?": False}},
        upsert = True
    )
